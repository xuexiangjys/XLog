...
public class CrashActivity extends AppCompatActivity {
    ...
    public static final String KEY_CRASH_MODEL = "key_crash_model";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setTheme(getIntent().getIntExtra(KEY_THEME_ID, R.style.SpiderManTheme_Light));
        model = getIntent().getParcelableExtra(KEY_CRASH_MODEL);
        if (model == null) {
            finish();
            return;
        }
    ...
    private File BitmapToFile(Bitmap bitmap) {
        ...
        String path = Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS)
                .getAbsolutePath();
        File imageFile = new File(path, "XLog_" + df.format(model.getTime()) + ".jpg");
        try {
